            ...
            for x in range(0, len(row)):
                ...
                if board[y][x] in "EG" and (y, x) not in hadTurn:
                    ...
                    if isNextToBadGuy(board, y, x, badGuy):
                        ...
                        print((y,x), unit, candidatePaths)
                        # try to move unit + attack
                        # path = bfs(board, (y, x), unit[0], getOpposing(unit[0]), units)
                        # print(y,x, unit, path)
                        if len(candidatePaths) > 0:
                            ...
                            if path is not None and len(path) > 1:
                                ...
                                ny, nx = path[1]
                                board[ny][nx] = unit[0]
                                attack(board, units, ny, nx, unit[0])
                                units[(ny, nx)] = unit
...
def attack(board, units, y, x, unitChar):
