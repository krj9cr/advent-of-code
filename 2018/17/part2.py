    ...
    while canFall or canSpread:
        ...
        while canSpread:
            ...
            if left is None and right is None and 0 <= mid[1]-1:
                ...
                if left:
                    replaceStillWater(board, left, "left")
                    canFall.add(left)
                if right:
                    replaceStillWater(board, right, "right")
                    canFall.add(right)
...
def replaceStillWater(board, start, direction):
    x, y = start
...
# still off by a bit, random middle sections were replaced with "|||||" when it
# should have been "~~~~~~", inspected output visually and changed
# it in a text editor, then did ctrl+F for "~"
def part2(path):
    ...
    printBoard(board)
    answer = countStillWater(board)
    print("part1:", answer)  # minus for start line(s)
