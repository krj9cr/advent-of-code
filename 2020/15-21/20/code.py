...
def part2(data):
    ...
    # RANDOMIZER ATTEMPT
    # tileNums = [ tileNum for tileNum, tile in data]
    # print(tileNums)
    # # while grid doesn't match
    # while True:
    #     # shuffle order of tiles
    #     random.shuffle(tileNums)
    #     # put randomized tileNums in the dim x dim grid
    #     rand = np.split(np.array(tileNums), dim)
    #     # randomly flip and rotate tiles
    #     randomTiles = getRandomTiles(data)
    #     # check grid matchos
    #     if checkGrid(rand, randomTiles):
    #         print(rand, randomTiles)
    #         print("HOH")
    #         break
    ...
    print("Upper left",biggoGriddo[0][0])

    # go along top
    for i in range(dim-1):
        currTileNum = biggoGriddo[0][i]
        nextTileNum, nextSide = matches[currTileNum][1].pop()
        # if nextSide == 0:
        biggoGriddo[0][i+1] = nextTileNum
        # print(biggoGriddo[0][i], next)
