        ...
        if (row1 == row2).all():
            ...
            if mirrored:
                ...
                if not vertical:
                    value *= 100
                # print(value)
                # check if we want to search for a different value
                if notValue and value != notValue:
                    # print("unique value", value, "!=", notValue)
                    break
                else:
                    # print("continuing")
                    continue
...
def find_mirror(grid, notValue=None):
    ...
    # print_2d_grid(grid)
    value = find_mirror_value(grid, vertical=False)
    ...
    if not value or (notValue and value == notValue):
        ...
        value = find_mirror_value(rotatedGrid, vertical=True)
    ...
    for gid in range(len(grids)):
        ...
        for j in range(len(grid)):
            ...
            for i in range(len(row)):
                ...
                value = find_mirror(newGrid, notValue=gridValue)
                # print(value)
                if value and value != gridValue:
                    print_2d_grid(grid)
